...
public class JawaBot extends PircBot
{
	...
	boolean handleJawaBotCommand( String fromChannel, String fromUser, String command )
   {
		...
		if( !isFromPrivateMessage && ( command.startsWith("take") || command.startsWith("keep") ) ) {
            ...
            if( "".equals( command ) ){
               ...
               boolean keep = false;
					...
					Reservation collidingReservation = resourceManager.bookResource(resName, fromUserNorm, fromDate, toDate);

					// Collision with another reservation.
					if( null != collidingReservation  /*&&  ! fromUserNorm.equals( collidingReservation.getForUser() ) */ ){
               ...
               // Bug: What if more reservations collide?
               // TODO: Must be solved on the resourceManager level...
               /*if( null != collidingReservation ){
                  keep = true;
                  fromDate = DateUtils.getEarlier(fromDate, collidingReservation.getFrom());
                  toDate = DateUtils.getLater(toDate, collidingReservation.getTo());
               }*/
                  ...
                  announceTakeOnMailingList( resourceManager.getResource(resName), new Reservation(fromUserNorm, fromDate, toDate) );
	 ...
	 */
	private void announceTakeOnMailingList( Resource resource, Reservation reservation ) {
      
      ConfigBean cnf = this.getConfig();

      String subject = String.format("Taking %s from %s to %s (JawaBot "+VERSION+")",
										resource.getName(),
