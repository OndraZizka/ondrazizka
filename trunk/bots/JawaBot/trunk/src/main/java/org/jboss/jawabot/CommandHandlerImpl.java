...
public class CommandHandlerImpl implements CommandHandler
{
         ...
         */
         ...
         try {
            ...
            // No collision or merged with older, we're free to go.
            {
               ...
               //announceTakeOnMailingList( resourceManager.findResourceByName(resNames), bookingResult, customComment );
   ...
   public CommandReply onLeave( CommandContext ctx, String params ) {
      ...
      if( "".equals( params )){
         reply.addReply("Leave what? Try 'leave all', 'list', 'list <resource>' or 'help'. (and 'list <user>' in the future).");
         return reply;
      }
      ...
      try {
         if( "all".equals(resName) ){
            ...
            if( leftResources.size() == 0 ){
               reply.addReply("Nothing to leave - you didn't own any resource.");
               return reply;
            }
   ...
   public CommandReply onList(CommandContext ctx, String params) {
      ...
      // Resource name?
      Resource res = this.jawaBot.getResourceManager().findResourceByName(param1);
      if( null != res ){
         try {
            String list = this.jawaBot.getResourceManager().printResourceReservations( res );
            reply.addReplies( list.split("\n") );
         }
         catch( JawaBotException ex ){
            reply.addReply( ex.getMessage() );
         }
      ...
      // Otherwise, assume a user name.
      else {
         String user = param1;
         if( "my".equals( param1 ) )
            user = ctx.fromUserNorm;

         List<ReservationWrap> reservs = this.jawaBot.getResourceManager().getReservationsForUser( user );

         if( reservs.size() == 0 ){
            reply.addReply( "No reservations for "+user+".");
         }
         else{
            for( ReservationWrap resv : reservs ) {
               reply.addReply( resv.asString() );
            }
         }
